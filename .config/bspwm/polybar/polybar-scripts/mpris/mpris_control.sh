...
show_menu_selector() {
    ...
    menu="$(rofi -theme ~/.config/rofi/launchers/tokyonight/tokyonight.rasi -sep "*" -dmenu -i -p "Choose Player" -location 0 -hide-scrollbar -line-padding 4 -padding 20 -kb-row-tab "" <<<${options})"
    menu=${menu:2}
    if [ menu == "Exit" ]; then
        return
    elif [ ${#menu} -eq 0 ]; then
